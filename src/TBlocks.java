...
public class TBlocks extends Blocks {
    private Block[] b;

    private int rotate;
        
        
    public TBlocks(int[][] playG, int index,int rotatePos, int x, int y) {
        super(index, 5000, rotatePos, x, y);
    ...
    public boolean down(int[][] playG) {
        ...
        if(rotate==0 || rotate==2){
            ...
            if (b[1]!=null && b[2]!=null && b[3]!=null){
                ...
                switch(indxCode){
                    case "A":
                        ret = Move.down3Blocks(playG, true, b[0], b[2], b[3]);              
                        break;                
                    case "B" :
                        ret = Move.down1Block(playG, b[1]);
                    ...
                    case "AB":
                        ...
                        break;
                    case "AC":
                        ret = Move.down2Blocks(playG, false, b[0], b[2]);
                        break;
                    case "BC":
                        ret = Move.down1Block(playG, b[1]);               
                        break;
    ...
    public void rotate(int[][] playG, int rotatePos, int x, int y){
        switch(rotatePos){
            ...
            case 1:
                if(y<0){y=0;}
                if(x>playG[0].length - 2){x=playG[0].length - 2;}

                if(    Move.isFreeGroundByCoord(playG, x, y, true, b[0]) 
                    && Move.isFreeGroundByCoord(playG, x+1, y+1, true, b[1])
                    && Move.isFreeGroundByCoord(playG, x+1, y, true, b[2])
                    && Move.isFreeGroundByCoord(playG, x+1, y-1, true, b[3])){    
